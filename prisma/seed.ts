...
async function main() {
  // Clean existing data
  await db.issueHistory.deleteMany()
  ...
  const issues = [
    ...
    },
    {
      ...
      description: 'The issue list pagination shows incorrect page counts when filters are applied. Need to fix the count calculation.',
      type: 'bug',
      priority: 'high',
      status: 'inprogress',
      estimate: 3,
  ...
  await db.issueHistory.createMany({
    data: [
      ...
      { action: 'comment_added', fromValue: '', toValue: 'Alice Smith', issueId: createdIssues[0].id, userId: alice.id },
    ],
  })
  ...
  await db.chatMessage.createMany({
    data: [
      ...
      { body: 'Great progress Alice! Based on the current sprint velocity, the team has completed 60% of planned story points. At this rate, Sprint 24 should finish on schedule. The main risk is FT-103 (pagination bug) which may need additional investigation time.', isAI: true, authorId: alice.id },
